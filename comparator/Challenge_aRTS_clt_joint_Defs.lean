...
/-! # Standalone extraction for `AlphaRAR.aRTS_clt_joint` — shared definitions
Definitions are copied verbatim; theorem proofs are replaced by `sorry`.
Auto-generated by Referee; split out of `Challenge_aRTS_clt_joint.lean` so that
`AlphaRAR.targetSqrtNVec` is elaborated in a module of its own, mirroring the original module
boundary (`PluginTargetCLT` vs `JointCLT`) that determines the naming of the auxiliary
`_proof_*` constants shared between definitions. -/

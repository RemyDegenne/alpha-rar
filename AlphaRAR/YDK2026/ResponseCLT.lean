...
lemma predVar_respArray_ae (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    ...
    (respArray h hY2 k vk).predVar n =ᵐ[P]
      fun ω ↦ (Var[id; ν k] * vk * n)⁻¹
        * (Var[id; ν k] * count (fun j ↦ armIndicator A k j ω) n) := by
  ...
  calc ∑ i ∈ Finset.range n, (P[fun ω ↦ (respIncr ν A Y k i ω) ^ 2
      ...
      = ∑ i ∈ Finset.range n, armIndicator A k i ω * Var[id; ν k] :=
        Finset.sum_congr rfl fun i _ ↦ hcs i
    _ = (∑ i ∈ Finset.range n, armIndicator A k i ω) * Var[id; ν k] := by rw [← Finset.sum_mul]
    _ = Var[id; ν k] * count (fun j ↦ armIndicator A k j ω) n := by rw [count]; ring
...
lemma condExp_indicator_comp (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P) (k : 𝓐) (i : ℕ)
  ...
  have hcondg : P[fun ω ↦ φ (Y i ω) | G] =ᵐ[P] fun ω ↦ (ν (A i ω))[φ] :=
    h.condExp_feedback_comp i hφ hint
...
lemma lindeberg_respArray_ae (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    (hY2 : ∀ n, MemLp (Y n) 2 P) (k : 𝓐) (vk : ℝ) (hvk : 0 < vk) (hVk : 0 < Var[id; ν k])
    (ε : ℝ) (hε : 0 < ε) (n : ℕ) :
  ...
  · have han : 0 < Var[id; ν k] * vk * (n : ℝ) :=
    ...
    have hsummand : ∀ i, (P[{ω | ε < |(respArray h hY2 k vk).d n i ω|}.indicator
          ...
          | IsAlgEnvSeq.filtrationAction h.measurable_action h.measurable_feedback i]) =ᵐ[P]
        fun ω ↦ (Var[id; ν k] * vk * (n : ℝ))⁻¹ * armIndicator A k i ω * ∫ x, φ x ∂(ν k) := by
      intro i
      have hFi : {ω | ε < |(respArray h hY2 k vk).d n i ω|}.indicator
            (fun ω ↦ ((respArray h hY2 k vk).d n i ω) ^ 2)
          = fun ω ↦ (Var[id; ν k] * vk * (n : ℝ))⁻¹ * (armIndicator A k i ω * φ (Y i ω)) := by
        funext ω
      ...
      have hconst := condExp_const_mul (P := P)
        ...
        (Var[id; ν k] * vk * (n : ℝ))⁻¹ (fun ω ↦ armIndicator A k i ω * φ (Y i ω))
  ...
  · filter_upwards [ae_all_iff.mpr (fun n ↦ lindeberg_respArray_ae h hY2 k vk hvk hVk ε hε n)]
    ...
    have hgoal : Tendsto (fun n : ℕ ↦ (Var[id; ν k] * vk * (n : ℝ))⁻¹ * T n
      ...
      · have hkn : (0 : ℝ) ≤ (Var[id; ν k] * vk * (n : ℝ))⁻¹ * T n :=
        ...
        calc (Var[id; ν k] * vk * (n : ℝ))⁻¹ * T n * count (fun j ↦ armIndicator A k j ω) n
...
lemma measurable_count_armIndicator (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P) (k : 𝓐) (n : ℕ) :
  ...
  simp only [count]
  exact Finset.measurable_sum _ fun j _ ↦ measurable_armIndicator h.measurable_action k j

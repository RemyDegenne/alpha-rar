...
(Condition **B**, giving continuity via `LipschitzWith.continuous`), the algorithm-level predicate
`IsARTS`, `α ∈ [0,1)`, the Condition **B** non-sparsity `hTpos` (`T > 0` on the attainable set),
and the first-order differentiability of `T` at `θ` with Jacobian matrix `G` (`hTderiv`, the
remaining Condition **B** input, exactly as in `clt_rho`) — the `√n`-scaled joint vector
`(√n(N_n/n - v), √n(ρ̂_n - v))` converges weakly to the block-Gaussian `𝒩(0, Ω)`, with every block
of `Ω` equal to `G · diag(armVar_k / v_k) · Gᵀ` and `v_k = T((ν_k)[id])_k` the limiting proportion.

This is the `aRTS` instantiation of the generic `AlphaRAR.clt_joint`: the `thm:LLN` consistencies
`θ̂ → θ` (`aRTS_theta_consistent`), `N/n → v` (`aRTS_proportion_tendsto`) and the non-sparsity
`0 < v_k` are all *derived* (as in `aRTS_prop_dev`), and the proportion-deviation hypothesis
`√n(N_n/n - ρ̂_n) →ₚ 0` is discharged from `aRTS_prop_dev` (part (i)) via the coordinatewise-`o_p`
assembly and the `√n`-scaling identity. -/
theorem aRTS_clt_joint [Fintype 𝓐] [DecidableEq 𝓐] [StandardBorelSpace 𝓐] [Nonempty 𝓐]
    ...
    Tendsto (β := ProbabilityMeasure (EuclideanSpace ℝ (𝓐 ⊕ 𝓐)))
      ...
      (𝓝 ⟨multivariateGaussian 0 (Matrix.fromBlocks
        (G * Matrix.diagonal (fun a ↦ armVar ν a / T (fun k' ↦ (ν k')[id]) a) * Gᵀ)
        (G * Matrix.diagonal (fun a ↦ armVar ν a / T (fun k' ↦ (ν k')[id]) a) * Gᵀ)
        (G * Matrix.diagonal (fun a ↦ armVar ν a / T (fun k' ↦ (ν k')[id]) a) * Gᵀ)
        (G * Matrix.diagonal (fun a ↦ armVar ν a / T (fun k' ↦ (ν k')[id]) a) * Gᵀ)),
          inferInstance⟩) := by
  have hT : Continuous T := hlip.continuous
  -- The `thm:LLN` consistencies, derived from the design bundle (as in `aRTS_prop_dev`).
  ...
  have hNconv : ∀ᵐ ω ∂P, ∀ a, Tendsto (fun n ↦ count (fun j ↦ armIndicator A a j ω) n / (n : ℝ))
      atTop (𝓝 (T (fun k' ↦ (ν k')[id]) a)) := by
    rw [ae_all_iff]
    exact fun a ↦ (aRTS_proportion_tendsto h hY2 hT hTnn hTsum hα hARTS hTpos a).mono
      fun ω hω ↦ hω.congr fun n ↦ by rw [count_indicator_eq_pullCount]
  -- The proportion-deviation fact `√n(N_n/n - ρ̂_n) →ₚ 0`, discharged from `aRTS_prop_dev`.
  have hprop : TendstoInMeasure P (fun n ω ↦ propSqrtNVec A (T (fun k' ↦ (ν k')[id])) n ω
    ...
    have hpd : ∀ k, IsLittleOpOne P (fun n ω ↦ ((pullCount A k n ω : ℝ)
      ...
      fun k ↦ aRTS_prop_dev h hY2 θ₀ T hTnn hTsum α hα hα1 hARTS hlip hTpos k

...
lemma predVar_wArray_ae (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    ...
    (wArray h hY2 wn).predVar n =ᵐ[P]
      fun ω ↦ ∑ a, wn n a ^ 2 * armVar ν a * count (fun j ↦ armIndicator A a j ω) n := by
  ...
  calc ∑ i ∈ Finset.range n, (P[fun ω ↦ (wIncr ν A Y (wn n) i ω) ^ 2
        | IsAlgEnvSeq.filtrationAction h.measurable_action h.measurable_feedback i]) ω
      = ∑ i ∈ Finset.range n, ∑ a, wn n a ^ 2 * (armIndicator A a i ω * armVar ν a) :=
        Finset.sum_congr rfl fun i _ ↦ hcs i
    _ = ∑ a, wn n a ^ 2 * armVar ν a * count (fun j ↦ armIndicator A a j ω) n := by
        rw [Finset.sum_comm]
        refine Finset.sum_congr rfl fun a _ ↦ ?_
...
lemma tendstoInMeasure_predVar_wArray (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    ...
    TendstoInMeasure P
      ...
      (fun _ ↦ ∑ a, w a ^ 2 * armVar ν a * ρ a) := by
...
lemma wLinComb_scaled_tendsto_gaussianReal (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    (hY2 : ∀ n, MemLp (Y n) 2 P) (w : 𝓐 → ℝ) (hνk : ∀ a, MemLp (fun x : ℝ ↦ x) 2 (ν a))
    {c : 𝓐 → ℕ → ℝ} (hc : ∀ a n, 0 ≤ c a n) (hc_atTop : ∀ a, Tendsto (c a) atTop atTop)
    {ρ : 𝓐 → ℝ} (hσ2 : 0 ≤ ∑ a, w a ^ 2 * armVar ν a * ρ a)
    (hNconv : ∀ᵐ ω ∂P, ∀ a, Tendsto (fun n ↦ count (fun j ↦ armIndicator A a j ω) n / c a n)
      atTop (𝓝 (ρ a))) :
    Tendsto (β := ProbabilityMeasure ℝ)
      ...
      atTop (𝓝 ⟨gaussianReal 0 (∑ a, w a ^ 2 * armVar ν a * ρ a).toNNReal, inferInstance⟩) := by
  have hmart := (wArray h hY2 (fun n a ↦ w a / Real.sqrt (c a n))).mart_clt
    (σ2 := ∑ a, w a ^ 2 * armVar ν a * ρ a) hσ2
...
lemma respMart_joint_tendsto_multivariateGaussian
    ...
    Tendsto (fun n : ℕ ↦ ProbabilityMeasure.map (⟨P, inferInstance⟩ : ProbabilityMeasure Ω)
      ...
      (𝓝 (ProbabilityMeasure.map
          (⟨multivariateGaussian 0 (Matrix.diagonal (fun a ↦ ρ a * armVar ν a)), inferInstance⟩
  ...
  have hs2nn : 0 ≤ ∑ a, t.ofLp a ^ 2 * armVar ν a * ρ a :=
  ...
  have hQ : ProbabilityMeasure.map
      ...
      = ⟨gaussianReal 0 (∑ a, t.ofLp a ^ 2 * armVar ν a * ρ a).toNNReal, inferInstance⟩ := by
    apply ProbabilityMeasure.toMeasure_injective
    simp only [ProbabilityMeasure.toMeasure_map, ProbabilityMeasure.coe_mk]
    change (multivariateGaussian 0 (Matrix.diagonal fun a ↦ ρ a * armVar ν a)).map
      (fun x ↦ (⟪x, t⟫ : ℝ)) = _

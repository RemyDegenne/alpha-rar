...
theorem qm_increments_resp (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
  ...
  have harmnn : ∀ k, 0 ≤ armVar ν k := fun k ↦ by rw [armVar]; exact variance_nonneg _ _
  have hC₀ : 0 ≤ ∑ k, armVar ν k := Finset.sum_nonneg (fun k _ ↦ harmnn k)
  have hincfam : ∀ k n, ∫ ω, (respMart ν A Y k (n + 1) ω - respMart ν A Y k n ω) ^ 2 ∂P
      ≤ ∑ k, armVar ν k := fun k n ↦ (integral_respMart_increment_sq_le h k n (hY2 n)).trans

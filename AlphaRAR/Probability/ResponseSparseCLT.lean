...
`Q_{n,k}/√N_{n,k} ⇒ 𝒩(0, V_k)`, where `V_k = armVar ν k`.
...
`Q_{n,k}/√N_{n,k} ⇒ 𝒩(0, V_k)`, where `V_k = armVar ν k`. No positivity of the limiting proportion
...
theorem respMart_selfNorm_anscombe_tendsto
    ...
    Tendsto (β := ProbabilityMeasure ℝ)
      ...
      (𝓝 (⟨gaussianReal 0 (armVar ν k).toNNReal, inferInstance⟩ : ProbabilityMeasure ℝ)) := by
  ...
  have hVar : Var[sampledClean Y D 0; P] = armVar ν k := by
...
lemma measurable_estimator_arm (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P) (k : 𝓐) (θ₀ : ℝ)
    (n : ℕ) :
    Measurable (fun ω ↦ estimator (fun j ↦ armIndicator A k j ω) (fun j ↦ Y j ω) θ₀ n) := by
...
`√N_{n,k}(θ̂_{n,k} - θ_k) ⇒ 𝒩(0, V_k)`, with `V_k = armVar ν k`. No positivity of the limiting
...
theorem estimator_sqrtN_anscombe_tendsto
    ...
    Tendsto (β := ProbabilityMeasure ℝ)
      ...
      atTop (𝓝 (⟨gaussianReal 0 (armVar ν k).toNNReal, inferInstance⟩ : ProbabilityMeasure ℝ)) := by
  set θ := (ν k)[id] with hθdef

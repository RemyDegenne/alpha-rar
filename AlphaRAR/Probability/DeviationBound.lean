...
lemma isLittleOpOne_const_div_sqrt [IsFiniteMeasure μ] (C : ℝ) :
    IsLittleOpOne μ (fun n (_ : Ω) ↦ C / Real.sqrt n) := by
...
lemma isLittleOpOne_max_div_sqrt_of_drift {c : ℝ} (hc : 0 < c)
    ...
    (hd : ∀ n ω, 0 ≤ d n ω)
    (hX : ∀ᶠ n in atTop, ∀ᵐ ω ∂μ, X n ω ≤ d n ω * (-c + A n ω) + B n ω * Real.sqrt n) :
    IsLittleOpOne μ (fun n ω ↦ max (X n ω / Real.sqrt n) 0) := by
  apply isLittleOpOne_of_tendsto_abs
  intro ε hε
  have hbound : ∀ᶠ n in atTop, μ {ω | ε ≤ |max (X n ω / Real.sqrt n) 0|}
    ...
    intro hω
    have hωval : ε ≤ |max (X n ω / Real.sqrt n) 0| := hω
    rw [abs_of_nonneg (le_max_right (X n ω / Real.sqrt n) 0)] at hωval
    ...
    have hXle : X n ω ≤ B n ω * Real.sqrt n := by linarith [hXω]
    have hXdiv : X n ω / Real.sqrt n ≤ B n ω := (div_le_iff₀ hsn).mpr hXle
    have hBlt : B n ω < ε := lt_of_abs_lt hBε
    have hmax : max (X n ω / Real.sqrt n) 0 < ε :=
...
lemma isLittleOpOne_max_of_decomp {c : ℝ} (hc : 0 < c)
    ...
    (hVM : IsLittleOpOne μ VM) (hWM : IsLittleOpOne μ (fun n ω ↦ WM n ω / Real.sqrt n))
    (hρbound : ∀ᶠ n in atTop, ∀ᵐ ω ∂μ, rhoterm n ω ≤ Vρ n ω * d n ω + Wρ n ω)
    (hVρ : IsLittleOpOne μ Vρ) (hWρ : IsLittleOpOne μ (fun n ω ↦ Wρ n ω / Real.sqrt n)) :
    IsLittleOpOne μ (fun n ω ↦ max (Uincr n ω / Real.sqrt n) 0) := by
  refine isLittleOpOne_max_div_sqrt_of_drift hc
    (A := fun n ω ↦ VM n ω + Vρ n ω + pert n ω)
    (B := fun n ω ↦ WM n ω / Real.sqrt n + Wρ n ω / Real.sqrt n)
  ...
  have hsn : (0 : ℝ) < Real.sqrt n := Real.sqrt_pos.mpr (by exact_mod_cast hn)
  have hB : (WM n ω / Real.sqrt n + Wρ n ω / Real.sqrt n) * Real.sqrt n = WM n ω + Wρ n ω := by
    field_simp
  ...
  change Uincr n ω ≤ d n ω * (-c + (VM n ω + Vρ n ω + pert n ω))
    + (WM n ω / Real.sqrt n + Wρ n ω / Real.sqrt n) * Real.sqrt n
  rw [hB, hexpand]
...
lemma isLittleOpOne_maxDev_of_le {Dev small Uincr : ℕ → Ω → ℝ}
    ...
    (hsmall : IsLittleOpOne μ (fun n ω ↦ max (small n ω / Real.sqrt n) 0))
    (hU : IsLittleOpOne μ (fun n ω ↦ max (Uincr n ω / Real.sqrt n) 0)) :
    IsLittleOpOne μ (fun n ω ↦ max (Dev n ω / Real.sqrt n) 0) := by
  refine IsLittleOpOne.of_abs_le_ae
    (Y := fun n ω ↦ max (small n ω / Real.sqrt n) 0 + max (Uincr n ω / Real.sqrt n) 0) ?_
    (hsmall.add hU)
  intro n
  ...
  · have hsn : (0 : ℝ) < Real.sqrt n := Real.sqrt_pos.mpr (by exact_mod_cast hn)
    have hdiv : Dev n ω / Real.sqrt n
        ≤ small n ω / Real.sqrt n + Uincr n ω / Real.sqrt n := by
      rw [← add_div]; gcongr
    refine max_le ?_ (by positivity)
    have hsle := le_max_left (small n ω / Real.sqrt n) 0
    have hUle := le_max_left (Uincr n ω / Real.sqrt n) 0
    linarith
...
lemma isLittleOpOne_dev_of_sum_zero {ι : Type*} [Fintype ι]
    ...
    (hfwd : ∀ j, IsLittleOpOne μ (fun n ω ↦ max (Dev j n ω / Real.sqrt n) 0)) (k : ι) :
    IsLittleOpOne μ (fun n ω ↦ Dev k n ω / Real.sqrt n) := by
  classical
  apply isLittleOpOne_of_abs
  have hrev : IsLittleOpOne μ (fun n ω ↦ max (-(Dev k n ω / Real.sqrt n)) 0) := by
    refine IsLittleOpOne.of_abs_le
      (Y := fun n ω ↦ ∑ j ∈ Finset.univ.erase k, max (Dev j n ω / Real.sqrt n) 0) ?_
      (isLittleOpOne_finset_sum fun j _ ↦ hfwd j)
    intro n ω
    rw [abs_of_nonneg (le_max_right _ _),
      abs_of_nonneg (Finset.sum_nonneg fun j _ ↦ le_max_right _ _)]
    have hneg : -(Dev k n ω / Real.sqrt n)
        = ∑ j ∈ Finset.univ.erase k, Dev j n ω / Real.sqrt n := by
  ...
  have hcomb : (fun n ω ↦ |Dev k n ω / Real.sqrt n|)
      = fun n ω ↦ max (Dev k n ω / Real.sqrt n) 0 + max (-(Dev k n ω / Real.sqrt n)) 0 := by

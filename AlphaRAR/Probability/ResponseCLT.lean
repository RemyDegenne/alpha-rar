...
`Q_{n,k}/√(V_k N_{n,k}) ⇒ 𝒩(0,1)`, where `V_k = armVar ν k` is the arm variance and
...
lemma sum_respIncr (k : 𝓐) (n : ℕ) :
    ∑ i ∈ Finset.range n, respIncr ν A Y k i = respMart ν A Y k n := rfl
...
lemma stronglyMeasurable_respIncr (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P) (k : 𝓐) (i : ℕ) :
  ...
  have hAm := IsAlgEnvSeq.measurable_action_filtrationAction_lt h.measurable_action
    h.measurable_feedback (Nat.lt_succ_self i)
  have hYm := IsAlgEnvSeq.measurable_feedback_filtrationAction_lt h.measurable_action
    h.measurable_feedback (Nat.lt_succ_self i)
...
noncomputable def respArray (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
  ...
  MartDiffArray.ofSeq (IsAlgEnvSeq.filtrationAction h.measurable_action h.measurable_feedback)
    (respIncr ν A Y k) (fun n ↦ armVar ν k * vk * n)
...
@[simp] lemma respArray_d (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    ...
    (respArray h hY2 k vk).d n i
      = fun ω ↦ (Real.sqrt (armVar ν k * vk * n))⁻¹ * respIncr ν A Y k i ω := rfl
...
lemma rowSum_respArray (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    ...
    (respArray h hY2 k vk).rowSum n
      = fun ω ↦ (Real.sqrt (armVar ν k * vk * n))⁻¹ * respMart ν A Y k n ω := by
...
lemma predVar_respArray_ae (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    (hY2 : ∀ n, MemLp (Y n) 2 P) (k : 𝓐) (vk : ℝ) (hvk0 : 0 ≤ vk) (hVk0 : 0 ≤ armVar ν k) (n : ℕ) :
    (respArray h hY2 k vk).predVar n =ᵐ[P]
      fun ω ↦ (armVar ν k * vk * n)⁻¹
        * (armVar ν k * count (fun j ↦ armIndicator A k j ω) n) := by
  have ha : ∀ n : ℕ, 0 ≤ armVar ν k * vk * (n : ℝ) := fun n ↦
  ...
  have hcondsq : ∀ i, P[fun ω ↦ (respIncr ν A Y k i ω) ^ 2
      ...
      fun ω ↦ armIndicator A k i ω * armVar ν k :=
  ...
  calc ∑ i ∈ Finset.range n, (P[fun ω ↦ (respIncr ν A Y k i ω) ^ 2
      ...
      = ∑ i ∈ Finset.range n, armIndicator A k i ω * armVar ν k :=
        Finset.sum_congr rfl fun i _ ↦ hcs i
    _ = (∑ i ∈ Finset.range n, armIndicator A k i ω) * armVar ν k := by rw [← Finset.sum_mul]
    _ = armVar ν k * count (fun j ↦ armIndicator A k j ω) n := by rw [count]; ring
...
lemma condExp_indicator_comp (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P) (k : 𝓐) (i : ℕ)
  ...
  have hcgint : Integrable (fun ω ↦ c ω * φ (Y i ω)) P := by
    have hform : (fun ω ↦ c ω * φ (Y i ω)) = {ω | A i ω = k}.indicator (fun ω ↦ φ (Y i ω)) := by
      funext ω
      simp only [hc, armIndicator, Set.indicator]
  ...
  have hcondg : P[fun ω ↦ φ (Y i ω) | G] =ᵐ[P] fun ω ↦ (ν (A i ω))[φ] :=
    condExp_feedback_comp h i hφ hint
  ...
  · rw [hak]
  · have hc0 : c ω = 0 := by
      rw [hc, armIndicator, Set.indicator_of_notMem (by simpa using hak)]
...
lemma lindeberg_respArray_ae (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    (hY2 : ∀ n, MemLp (Y n) 2 P) (k : 𝓐) (vk : ℝ) (hvk : 0 < vk) (hVk : 0 < armVar ν k)
    (ε : ℝ) (hε : 0 < ε) (n : ℕ) :
    (respArray h hY2 k vk).lindeberg n ε =ᵐ[P]
      fun ω ↦ (armVar ν k * vk * n)⁻¹
        * (∫ x, {x | ε * Real.sqrt (armVar ν k * vk * n) < |x - (ν k)[id]|}.indicator
  ...
  · have han : 0 < armVar ν k * vk * (n : ℝ) :=
      mul_pos (mul_pos hVk hvk) (by exact_mod_cast hn0)
    have hspos : 0 < Real.sqrt (armVar ν k * vk * (n : ℝ)) := Real.sqrt_pos.mpr han
    have hs2 : Real.sqrt (armVar ν k * vk * (n : ℝ)) ^ 2 = armVar ν k * vk * (n : ℝ) :=
      Real.sq_sqrt han.le
    set θ := (ν k)[id] with hθ
    set s := Real.sqrt (armVar ν k * vk * (n : ℝ)) with hs
    ...
    have hsummand : ∀ i, (P[{ω | ε < |(respArray h hY2 k vk).d n i ω|}.indicator
          (fun ω ↦ ((respArray h hY2 k vk).d n i ω) ^ 2)
          | IsAlgEnvSeq.filtrationAction h.measurable_action h.measurable_feedback i]) =ᵐ[P]
        fun ω ↦ (armVar ν k * vk * (n : ℝ))⁻¹ * armIndicator A k i ω * ∫ x, φ x ∂(ν k) := by
      intro i
      have hFi : {ω | ε < |(respArray h hY2 k vk).d n i ω|}.indicator
            (fun ω ↦ ((respArray h hY2 k vk).d n i ω) ^ 2)
          = fun ω ↦ (armVar ν k * vk * (n : ℝ))⁻¹ * (armIndicator A k i ω * φ (Y i ω)) := by
        funext ω
      ...
      have hconst := condExp_const_mul (P := P)
        ...
        (armVar ν k * vk * (n : ℝ))⁻¹ (fun ω ↦ armIndicator A k i ω * φ (Y i ω))
    ...
    rw [show (∑ i ∈ Finset.range n, armIndicator A k i ω)
        = count (fun j ↦ armIndicator A k j ω) n from rfl]
    ring
...
lemma tendstoInMeasure_lindeberg_respArray (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    (hY2 : ∀ n, MemLp (Y n) 2 P) (k : 𝓐) (vk : ℝ) (hvk : 0 < vk) (hVk : 0 < armVar ν k)
    (hνk : MemLp (fun x : ℝ ↦ x) 2 (ν k)) :
    ∀ ε, 0 < ε → TendstoInMeasure P (fun n ↦ (respArray h hY2 k vk).lindeberg n ε) atTop 0 := by
  intro ε hε
  have ha_nn : ∀ n : ℕ, (0 : ℝ) ≤ armVar ν k * vk * (n : ℝ) := fun n ↦
  ...
  have hc : Tendsto (fun n : ℕ ↦ ε * Real.sqrt (armVar ν k * vk * (n : ℝ))) atTop atTop :=
  ...
  obtain ⟨T, hT⟩ : ∃ T : ℕ → ℝ, ∀ n, T n = ∫ x,
      {x | ε * Real.sqrt (armVar ν k * vk * (n : ℝ)) < |x - (ν k)[id]|}.indicator
  ...
  · exact ((measurable_const.mul (hcount_meas n)).aestronglyMeasurable).congr
      (lindeberg_respArray_ae h hY2 k vk hvk hVk ε hε n).symm
  · filter_upwards [ae_all_iff.mpr (fun n ↦ lindeberg_respArray_ae h hY2 k vk hvk hVk ε hε n)]
      with ω hω
    have hcnn : ∀ n, (0 : ℝ) ≤ count (fun j ↦ armIndicator A k j ω) n := fun n ↦
      Finset.sum_nonneg fun j _ ↦ armIndicator_nonneg A k j ω
    have hcle : ∀ n, count (fun j ↦ armIndicator A k j ω) n ≤ (n : ℝ) := by
      intro n
      rw [count]
      calc ∑ j ∈ Finset.range n, armIndicator A k j ω
          ≤ ∑ _j ∈ Finset.range n, (1 : ℝ) :=
            Finset.sum_le_sum fun j _ ↦ armIndicator_le_one A k j ω
        _ = (n : ℝ) := by simp
    have hgoal : Tendsto (fun n : ℕ ↦ (armVar ν k * vk * (n : ℝ))⁻¹ * T n
        * count (fun j ↦ armIndicator A k j ω) n) atTop (𝓝 0) := by
      refine squeeze_zero (fun n ↦ ?_) (fun n ↦ ?_) hb
      · exact mul_nonneg (mul_nonneg (inv_nonneg.mpr (ha_nn n)) (htail_nn n)) (hcnn n)
      · have hkn : (0 : ℝ) ≤ (armVar ν k * vk * (n : ℝ))⁻¹ * T n :=
          mul_nonneg (inv_nonneg.mpr (ha_nn n)) (htail_nn n)
        calc (armVar ν k * vk * (n : ℝ))⁻¹ * T n * count (fun j ↦ armIndicator A k j ω) n
            ≤ (armVar ν k * vk * (n : ℝ))⁻¹ * T n * (n : ℝ) :=
              mul_le_mul_of_nonneg_left (hcle n) hkn
          _ ≤ (armVar ν k * vk)⁻¹ * T n := by
...
lemma tendstoInMeasure_predVar_respArray (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    (hY2 : ∀ n, MemLp (Y n) 2 P) (k : 𝓐) (vk : ℝ) (hvk : 0 < vk) (hVk : 0 < armVar ν k)
    (hNconv : ∀ᵐ ω ∂P,
      Tendsto (fun n ↦ count (fun j ↦ armIndicator A k j ω) n / (n : ℝ)) atTop (𝓝 vk)) :
    TendstoInMeasure P (fun n ↦ (respArray h hY2 k vk).predVar n) atTop (fun _ ↦ (1 : ℝ)) := by
  have hV0 : armVar ν k ≠ 0 := hVk.ne'
  ...
  · exact ((measurable_const.mul
      ...
      (predVar_respArray_ae h hY2 k vk hvk.le hVk.le n).symm
  · have hae := ae_all_iff.mpr (fun n ↦ predVar_respArray_ae h hY2 k vk hvk.le hVk.le n)
    filter_upwards [hae, hNconv] with ω hpred hconv
    refine Tendsto.congr (fun n ↦ (hpred n).symm) ?_
    have hlim : Tendsto (fun n ↦ count (fun j ↦ armIndicator A k j ω) n / (n : ℝ) * vk⁻¹) atTop
...
lemma respMart_div_sqrt_tendsto_gaussianReal (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    ...
    Tendsto (β := ProbabilityMeasure ℝ)
      (fun n : ℕ ↦ (⟨P.map (fun ω ↦
          (Real.sqrt (armVar ν k * vk * (n : ℝ)))⁻¹ * respMart ν A Y k n ω),
  ...
  have hmart := (respArray h hY2 k vk).mart_clt (σ2 := 1) zero_le_one
    (tendstoInMeasure_predVar_respArray h hY2 k vk hvk hVk hNconv)
    (tendstoInMeasure_lindeberg_respArray h hY2 k vk hvk hVk hνk)
...
lemma respMart_selfNorm_tendsto_gaussianReal (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
    ...
    Tendsto (β := ProbabilityMeasure ℝ)
      (fun n : ℕ ↦ (⟨P.map (fun ω ↦
          (Real.sqrt (armVar ν k * count (fun j ↦ armIndicator A k j ω) n))⁻¹
      ...
      atTop (𝓝 ⟨gaussianReal 0 1, inferInstance⟩) := by
  have hRmeas : ∀ n : ℕ, AEMeasurable (fun ω ↦ Real.sqrt (armVar ν k * vk * (n : ℝ))
      * (Real.sqrt (armVar ν k * count (fun j ↦ armIndicator A k j ω) n))⁻¹) P :=
  ...
  have hRtendsto : TendstoInMeasure P (fun (n : ℕ) ω ↦ Real.sqrt (armVar ν k * vk * (n : ℝ))
      * (Real.sqrt (armVar ν k * count (fun j ↦ armIndicator A k j ω) n))⁻¹) atTop
      (fun _ ↦ (1 : ℝ)) := by
    ...
    · have hNpos : (0 : ℝ) < count (fun j ↦ armIndicator A k j ω) n :=
      ...
      have hsV : Real.sqrt (armVar ν k) ≠ 0 := Real.sqrt_ne_zero'.mpr hVk
  ...
  have hXR : ∀ n : ℕ, (fun ω ↦ (Real.sqrt (armVar ν k * vk * (n : ℝ)))⁻¹ * respMart ν A Y k n ω
        * (Real.sqrt (armVar ν k * vk * (n : ℝ))
          * (Real.sqrt (armVar ν k * count (fun j ↦ armIndicator A k j ω) n))⁻¹))
      = fun ω ↦ (Real.sqrt (armVar ν k * count (fun j ↦ armIndicator A k j ω) n))⁻¹
          * respMart ν A Y k n ω := by
    intro n
    funext ω
    rcases Nat.eq_zero_or_pos n with h0 | h0
    · subst h0; simp [respMart]
    · have hne : Real.sqrt (armVar ν k * vk * (n : ℝ)) ≠ 0 :=
        Real.sqrt_ne_zero'.mpr (mul_pos (mul_pos hVk hvk) (by exact_mod_cast h0))
      have hrw : (Real.sqrt (armVar ν k * vk * (n : ℝ)))⁻¹ * respMart ν A Y k n ω
          * (Real.sqrt (armVar ν k * vk * (n : ℝ))
            * (Real.sqrt (armVar ν k * count (fun j ↦ armIndicator A k j ω) n))⁻¹)
          = ((Real.sqrt (armVar ν k * vk * (n : ℝ)))⁻¹ * Real.sqrt (armVar ν k * vk * (n : ℝ)))
            * ((Real.sqrt (armVar ν k * count (fun j ↦ armIndicator A k j ω) n))⁻¹

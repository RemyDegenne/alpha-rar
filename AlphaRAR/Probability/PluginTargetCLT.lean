...
theorem clt_rho_of_tendstoInMeasure
  ...
  set μ' : Measure (EuclideanSpace ℝ 𝓐) :=
    multivariateGaussian 0 (Matrix.diagonal fun a ↦ armVar ν a / v a) with hμ'
  ...
  have hlim : μ'.map (fun x ↦ g (x, 0))
      = multivariateGaussian 0 (G * Matrix.diagonal (fun a ↦ armVar ν a / v a) * Gᵀ) := by
  ...
  have heq : (⟨multivariateGaussian 0 (G * Matrix.diagonal (fun a ↦ armVar ν a / v a) * Gᵀ),

...
lemma aRTS_g_littleOp [Finite 𝓐] (h : IsAlgEnvSeq A Y alg (stationaryEnv ν) P)
  ...
  have hSOp : IsBigOpOne P (fun n ω ↦ Sseq n ω / √n) := by
    refine isBigOpOne_sup'_abs_div_sqrt (martingale_respMart h hint k'')
      ...
      (ae_of_all _ fun ω ↦ by simp [respMart, Finset.range_zero]) (armVar ν k'')
      (by rw [armVar]; exact variance_nonneg _ _) ?_ ?_
...
theorem aRTS_prop_dev [Fintype 𝓐] [DecidableEq 𝓐] [StandardBorelSpace 𝓐] [Nonempty 𝓐]
  ...
  have hMmart : ∀ k', Martingale (assignMart (fun j ↦ armIndicator A k' j) ℱ P) ℱ.shiftDown P :=
    fun k' ↦ martingale_assignMart (stronglyAdapted_armIndicator h k')
  ...
  have hMdelta : ∀ k' n, ∀ᵐ ω ∂P,
    ...
    abs_assignMart_succ_sub_le (integrable_armIndicator h k')

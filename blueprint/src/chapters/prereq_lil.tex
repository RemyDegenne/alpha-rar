...
\lean{AlphaRAR.ae_eventually_le_sqrt_predQuadVar_mul_log}\leanok
...
\lean{AlphaRAR.ae_eventually_le_sqrt_nat_mul_log}\leanok
...
\lean{AlphaRAR.measure_exists_truncRespMart_block}\leanok
...
$M^N_m:=M_{\min(m,N)}$ (\texttt{stopMart}), which is a martingale
(\texttt{martingale\_stopMart}), whose increments vanish past $N$ hence are globally bounded by
$c$, and whose predictable quadratic variation agrees with $\langle M\rangle$ below the horizon
(\texttt{predQuadVar\_stopMart\_of\_le}). Here it is used with $N=2^k$, $c=c_k=2\sqrt{2^k}$, the
...
\lean{AlphaRAR.summable_block_bound}\leanok
...
AlphaRAR.ae_eventually_truncRespMart_le_sqrt_nat_mul_log}\leanok
...
\lean{AlphaRAR.ae_eventually_respMart_le_sqrt_nat_mul_log}\leanok
\uses{lem:trunc_tail_const, lem:trunc_drift, lem:trunc_mart_lil}
$M_n = O(\sqrt{n\log n})$ a.s.
\end{lemma}
\begin{proof}\leanok
\uses{lem:trunc_tail_const, lem:trunc_drift, lem:trunc_mart_lil}

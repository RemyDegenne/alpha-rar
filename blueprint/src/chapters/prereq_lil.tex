...
\begin{proof}
\uses{lem:lil_freedman}
Fix $\eta>1$ and set $v_k = \eta^k$. Apply Lemma~\ref{lem:lil_freedman} on the
event $\{\langle M\rangle_n\in[v_{k-1},v_k]\}$ with
$\lambda_k=(1+\varepsilon)\sqrt{2v_k\log\log v_k}$: the resulting probabilities
are summable in $k$, so Borel--Cantelli implies that a.s.\ only finitely many of
the events $\{M_n \ge \lambda_k,\ \langle M\rangle_n \le v_k\}$ occur. Letting

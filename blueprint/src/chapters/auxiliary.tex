...
The key point is again the filtration convention. $Q_{\cdot,k}$ is a martingale not only for
$\filt$ but for the half-step-finer filtration
$\mathcal{K}_n := \mathcal{G}_{n+1} = \filt_n\vee\sigma(A_{n+1})$ (\texttt{shiftFiltration} of
\texttt{filtrationAction}), because its increment is unpredictable \emph{even given the current
assignment} (\texttt{condExp\_respMart\_increment\_filtrationAction}). For that filtration the
paper's $V_k N_{n,k}$ is the \emph{ordinary} predictable quadratic variation
(Definition~\ref{def:pred_qv}): its increment is
$\E[(\Delta Q_{m+1,k})^2\mid\mathcal{K}_m] = \E[(\Delta Q_{m+1,k})^2\mid\mathcal{G}_{m+1}]
= V_k X_{m+1,k}$ (Lemma~\ref{lem:Q_cond_second_moment}), and summing over $m<n$ gives
$V_k\sum_{m\le n} X_{m,k} = V_k N_{n,k}$ --- the indicator survives the sum because it is
$\mathcal{K}_m$-measurable. That $Q^2-\langle Q\rangle$ is a martingale (for $\mathcal{K}$) is
Lemma~\ref{lem:qv_mart}. The sole hypothesis is finite arm variance (Condition~\textbf{A}),
formalized as $\xi_{\cdot,k}\in L^2$.
\end{proof}
...
Equivalently, $Q_{\cdot,k}\,Q_{\cdot,j}$ is a martingale (for the filtration $\mathcal{K}$).
\end{lemma}
...
$Q_{\cdot,k}$ is a martingale (for $\mathcal{K}$) with $Q_{0,k}=0$, and its increment second
moments are bounded by the arm variance: taking expectations in
Lemma~\ref{lem:Q_cond_second_moment} via the tower property,
$\E[(\Delta Q_{n+1,k})^2] = \E[X_{n+1,k}\,V_k] = V_k\,\Prob(X_{n+1,k}=1) \le V_k$

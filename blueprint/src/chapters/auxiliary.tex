...
For $k \ne j$, the cross variation vanishes:
$\langle Q_{\cdot,k}, Q_{\cdot,j}\rangle_n = 0$.
\end{lemma}

\begin{proof}
\uses{lem:qv_orthogonal}
The increments $\Delta Q_{\cdot,k}$ and $\Delta Q_{\cdot,j}$ are conditionally orthogonal
because $X_{m,k}X_{m,j} = 0$ for $k \ne j$ (each patient is assigned to exactly one arm), so
every cross term $\Delta Q_{m,k}\,\Delta Q_{m,j}$ is identically zero.
\end{proof}
